	...
	try{
		...
		if (response.data) {
			let data = [];
				...
				data.push({item:"Architecture",value:response.data['Architecture']});
				data.push({item:"CPU op-mode(s)",value:response.data['CPU op-mode(s)']});
...
useEffect(()=> {
  ...
  return (
    <Fragment>
		 <div className="card">
		{cpuData && cpuData.map((item, index) => (
